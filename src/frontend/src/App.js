...
function App() {
  ...
  const calculate = async (operation) => {
    ...
    try {
      ...
      // response = await axios.post(`${apiUrl}/add`, operationDto);

      // отправка запроса для выбранной операции
      switch (operation) {
        case 'add':
          response = await axios.post(`${appAPI}/api/calculator/add`, operationDto);
          break;
        case 'subtract':
          response = await axios.post(`${appAPI}/api/calculator/subtract`, operationDto);
          break;
        case 'multiply':
          response = await axios.post(`${appAPI}/api/calculator/multiply`, operationDto);
          break;
        case 'divide':
          response = await axios.post(`${appAPI}/api/calculator/divide`, operationDto);
          break;
        default:
